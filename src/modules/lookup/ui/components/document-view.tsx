    ...
    if (search != null && search !== "") {
      const getData = async () => {
        const data = await fetch(
          `${process.env.NEXT_PUBLIC_MEDIBRAIN_API_BASE_URL}/document/query`,
          {
            method: "POST",
            body: JSON.stringify({
      ...
      <ScrollArea className="mx-5 rounded-md h-84 border-black border">
        {!results || results.length === 0 ? (
          ...
          results.map((item, index) => (
            <div
              key={index}
              className="border-b my-2 p-4 rounded-lg gap-y-2 hover:bg-gray-300 hover:cursor-pointer "
            >
              <a
                target="_blank"
                href={`${process.env.NEXT_PUBLIC_SEAWEED_BASE_URL}/${item.fid}`}
              ...
              >
                <div className="flex flex-row justify-between">
                  ...
                  <h4>
                    ...
                    <span className="">{item.keywords}</span>
                  </h4>
                </div>
