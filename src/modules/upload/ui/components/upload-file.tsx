...
const UploadFile = () => {
    ...
    if (hasFile) {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_STIRLING_API_BASE_URL}/analysis/document-properties`,
          {
            method: "POST",
            headers: {
  ...
  async function onSubmit(data: DocumentMeta) {
    let endpoint = `${process.env.NEXT_PUBLIC_MEDIBRAIN_API_BASE_URL}/document/upload`;
    const formData = new FormData();
    formData.append("fileInput", selectedFile!);

    try {
      if (isDirty) {
        endpoint = `${process.env.NEXT_PUBLIC_MEDIBRAIN_API_BASE_URL}/document/upload-with-edit`;
  ...
  return (
    <div className="flex flex-col justify-center max-w-md mx-auto items-center text-center">
      ...
      <div
        className={
          fileProvided
            ? "block animate-in fade-in duration-300 w-[50dvw]"
            : "hidden"
        }
      >
        <div
          className={
            !isLoading
              ? "block border rounded-lg p-4 bg-card space-y-4"
              : "hidden"
          }
        >
        ...
        <div
          className={
            isLoading && fileProvided ? " flex justify-center" : "hidden"
          }
        >
